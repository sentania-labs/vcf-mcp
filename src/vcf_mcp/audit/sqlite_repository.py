    ...
    def open(self) -> None:
        ...
        try:
            ...
            if existing is None:
                ...
                connection.execute(
                    "ALTER TABLE audit_records ADD COLUMN authorization_mode TEXT"
                )
                connection.execute(
                    "ALTER TABLE audit_records ADD COLUMN key_owner TEXT"
                )
                connection.execute(
                    "UPDATE schema_version SET version = ?", (SCHEMA_VERSION,)
                )
                ...
                connection.execute(
                    ...
                    _row_from_record(
                        AuditRecord(
                            ...
                            authorization_mode=attempt.authorization_mode,
                            key_owner=attempt.key_owner,
                        )
                    ),
                )
            connection.execute("COMMIT")

...
def create_production_app() -> Starlette:
    ...
    if configuration_ready:
        try:
            ...
            verified = pack_trust_manager.verify_active_at_startup()
            packs = load_backend_packs(
                operator_path=DEFAULT_OPERATOR_PACKS_PATH,
                verified_operator_digests=verified,
            )
            runtime_repository.set_grantable_scopes_at_startup(
                implemented_scopes(packs)
            )

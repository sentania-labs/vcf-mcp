...
from vcf_mcp.backend_packs import PackTool
from vcf_mcp.contracts import ConfigurationGeneration, TargetId, TargetRecord
from vcf_mcp.vcf.caps import MAX_UPSTREAM_RESPONSE_BYTES
from vcf_mcp.vcf.client import TargetCredentials, build_tls_verifier
    ...
    def __init__(
        ...
        *,
        ...
        self._upstream_control = upstream_control or UpstreamControl(
            backend_name=target.backend.value,
            target_id=str(target.id),
            max_concurrency=int(declared_caps.get("max_concurrency", 8)),
            max_429_retries=int(declared_caps.get("max_429_retries", 3)),
        )
